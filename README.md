...
```bash
sam-serverless-template$ sam build
```
...
```bash
sam-serverless-template$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-serverless-template$ sam local start-api
sam-serverless-template$ curl http://localhost:3000/
```
...
```bash
sam-serverless-template$ sam logs -n HelloWorldFunction --stack-name sam-serverless-template --tail
```
...
```bash
sam-serverless-template$ cd hello-world
...
```bash
sam delete --stack-name sam-serverless-template
```
